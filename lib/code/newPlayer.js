"use strict";

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports["default"] = void 0;

function _classCallCheck(instance, Constructor) { if (!(instance instanceof Constructor)) { throw new TypeError("Cannot call a class as a function"); } }

function _defineProperties(target, props) { for (var i = 0; i < props.length; i++) { var descriptor = props[i]; descriptor.enumerable = descriptor.enumerable || false; descriptor.configurable = true; if ("value" in descriptor) descriptor.writable = true; Object.defineProperty(target, descriptor.key, descriptor); } }

function _createClass(Constructor, protoProps, staticProps) { if (protoProps) _defineProperties(Constructor.prototype, protoProps); if (staticProps) _defineProperties(Constructor, staticProps); return Constructor; }

// eslint-disable-next-line no-unused-vars
var Player = /*#__PURE__*/function () {
  function Player(name) {
    _classCallCheck(this, Player);

    this.name = name;
    this.pastHits = [];
    this.currentMove = null;
  }

  _createClass(Player, [{
    key: "callRandomNumber",
    value: function callRandomNumber(val) {
      return this.constructor.randomNumber(val);
    }
  }, {
    key: "randomMove",
    value: function randomMove() {
      var coord = this.callRandomNumber(100);
      if (this.pastHits.some(function (pastHit) {
        return pastHit === coord;
      })) return this.callRandomNumber(100);
      this.pastHits.push(coord);
      this.currentMove = coord;
      return coord;
    }
  }, {
    key: "clearPastHits",
    value: function clearPastHits() {
      this.pastHits = [];
    }
  }], [{
    key: "randomNumber",
    value: function randomNumber(val) {
      return Math.floor(Math.random() * val);
    }
  }]);

  return Player;
}();

var _default = Player;
exports["default"] = _default;